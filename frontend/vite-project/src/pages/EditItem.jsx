...
function EditItem() {
  ...
  const handleGetItemById = async () => {
    try {
      ...
      console.log("fetch error:", error.response?.status, error.response?.data)
    }
  }
      ...
      <div className="max-w-lg w-full bg-white shadow-xl rounded-2xl p-8 border border-orange-100">
        ...
        <form className='space-y-5' onSubmit={handleSubmit}>
            <div>
                ...
                <input type="text" placeholder="Enter Food Name" className='w-full px-4 py-2
            ...
            <div>
             ...
             {frontendImage && <div className="mt-4">
              <img src={frontendImage} alt="" className='w-full h-48 object-cover rounded-lg border'></img>
            </div> }
              ...
              <div>
                ...
                onChange={(e)=>setCategory(e.target.value)} value={category} >
                      ...
                      {categories.map ((cate,index)=>{
                       return (<option value={cate} key={index}>{cate}</option>)
                      })}
