...
function Nav() {
  ...
  return (
    <div className='w-full h-[60px] md:h-[80px] flex items-center justify-between
      px-[14px] md:px-[20px] md:justify-center md:gap-[30px]
      fixed left-0 top-0 z-[9999] bg-[#fff9f6] shadow-sm overflow-visible'>
        
        {showSearch && <div className='flex w-[90%]  h-[50px] bg-white shadow-xl
        rounded-lg items-center gap-[12px] fixed top-[80px] left-[5%]'>
        <div className='flex items-center w-[30%] overflow-hidden gap-[8px] px-[10px]
      ...
      {/* Logo */}
      <h1 className='text-2xl md:text-3xl font-bold text-[#ff4d2d] shrink-0'>Zlinkit</h1>

      {/* Search bar — hidden on mobile */}
      <div className='hidden md:flex w-[60%] lg:w-[40%] h-[50px] bg-white shadow-xl
      ...
      <div className='flex items-center gap-3'>
        ...
        {showSearch?<RxCross2 size={22} className='text-[#ff4d2d] md:hidden' onClick={()=>setShowSearch(false)}/>:<IoSearch size={22} className='text-[#ff4d2d] md:hidden' onClick={()=>setShowSearch(true)} />}
        

        {/* Cart */}
        <div className='relative cursor-pointer'>
          <BiCart size={24} className='text-[#ff4d2d]' />
          <span className='absolute right-[-8px] top-[-10px] text-xs font-semibold
            text-[#ff4d2d] leading-none'>0</span>
        </div>
        ...
        {/* Avatar */}
        <div className='w-[36px] h-[36px] md:w-[40px] md:h-[40px] rounded-full
          flex items-center justify-center bg-[#ff4d2d]
          text-white text-[16px] font-semibold cursor-pointer shrink-0'onClick={()=>setPopup(prev=>!prev)}>
          {initial}
        </div>

      {popup && <div className='fixed top-[80px] right-[10px] md:right-[10px] lg:right-[25%]
